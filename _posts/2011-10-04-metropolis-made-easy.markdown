...
There's a lot to say about coming up with smarter proposals that help the sampler to efficiently explore parameter space. Here, I'll just mention one element of this: trying different jump sizes in the Gaussian proposal used above.  The figures above were generated by a sampler than generated proposals from a Gaussian with a standard deviation of 0.5. In the figures below, I show the result of running the sampler for 10,000 steps with each of 8 different Gaussian proposal widths, from .01 up to 5.
...
Changing the jump size has two main effects. First, and most obviously, subsequent steps are closer together when the average jump size is small - i.e., it takes more steps to explore the parameter space with small jumps.  Notice how little of the space is explored with 10,000 steps at the smallest proposal sizes shown above.  With large jumps, a single step can carry the sampler all the way across the region of interest.

This might suggest that large jumps are better, except that a second effect must be taken into account. Namely, large jumps are much less likely to be accepted (and, conversely, most small jumps will be accepted).  Intuitively, a large jump will usually represent a move to a part of parameter space which is lower in probability -- in fact, once the sampler has found a region of high probability, *most* large jumps will result in a much worse situation than the current one.  You can see this in the large-jump images above: some of the dots are very dark, indicating that the sampler remained at this location for many steps before a proposal was accepted.  As the parameter space becomes higher dimensional, this problem only gets worse.

As a very general rule of thumb, we want to find a jump size that results in an **acceptance rate** of 30 to 70%. While the exact number isn't crucial, we want to be sure that our acceptance rate isn't at either extreme.

## Next steps: generalizations and extensions

In some sense, we've only scratched the surface of Metropolis sampling, but I don't think that's the best way of looking at it.  In fact, our simple example contains all of the key elements -- the initial state, the proposal distribution, the target distribution, and the parameters over which inference is performed. Each of these elements can become significantly more complex, but their relationship to one another, and the overall structure of the sampler, remains the same.

That said, here are a few of the ways in which Metropolis and other MCMC samplers typically become more complex or sophisticated as our demands on them increase:

1. The target distribution can become much more complex. In particular, the number of parameters can increase dramatically, meaning that we need to explore a very high-dimensional space. In addition, some of the parameters might be vector valued, discrete, and so on (not just continuous).  This typically leads to…
...
3. Choosing a good initial state can become a project unto itself. In high-dimensional situations, the "peaks" of the target distribution can be very hard to find, and using a naive starting point might lead to the sampler requiring many steps to find these regions.
